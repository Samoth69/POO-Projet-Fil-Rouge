...
//		System.out.println(getRender(pieces));	
    ...
    private static String getRender(Collection<PieceModel> pieces) {
        ...
        for (PieceModel piece : pieces) {
            st += '[';
            st += piece.getPieceColor() == PieceSquareColor.BLACK ? 'B' : 'W';
            st += '[';
            st += piece.getLigne();
            st += ',';
            st += piece.getColonne();
        ...
        /*
        st += '\n';
        ArrayList<PieceModel> pm = new ArrayList<>(pieces);

        Collections.sort(pm); Comment Ruquier retire sa
        //Collections.sort(pm, new PieceComparator());

        for (PieceModel piece : pm) {
            st += '[';
            st += piece.getPieceColor() == PieceSquareColor.BLACK ? 'B' : 'W';
            st += '[';
            st += piece.getLigne();
...
class PieceComparator implements Comparator<PieceModel> {
    ...
    public int compare(PieceModel o1, PieceModel o2) {
        ...
        if (o1.getColonne() < o2.getColonne()) {
            ...
            ret = Integer.compare(o1.getLigne(), o2.getLigne());
        }
